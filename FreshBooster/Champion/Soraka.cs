    ...
    class Soraka
    {
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                if (QTarget != null && QTarget.Position.Distance(Player.Position) <= 480)
                {
                    _Q.SetSkillshot(0.3f, 210, 1100, false, SkillshotType.SkillshotCircle);
                }
                else
                {
                    _Q.SetSkillshot(0.5f, 210, 1100, false, SkillshotType.SkillshotCircle);
                ...
                var WTarget = HeroManager.Allies.OrderBy(f => f.Health)
                    ...
                    && !f.InFountain(Utility.FountainType.OwnFountain) && !f.IsDead && !f.IsZombie);
                ...
                {
                    if (_MainMenu.Item("Soraka_CUse_E").GetValue<bool>())
                        if (_E.IsReady())
                    ...
                    if (_MainMenu.Item("Soraka_CUse_Q").GetValue<bool>())
                        if (_Q.IsReady())
                ...
                {
                    if (_MainMenu.Item("Soraka_HUse_E").GetValue<bool>())
                        if (_E.IsReady())
                            if (_E.GetPrediction(ETarget).Hitchance >= HitChance.Medium)
                                _E.CastIfHitchanceEquals(ETarget, _E.GetPrediction(ETarget).Hitchance, true);
                    if (_MainMenu.Item("Soraka_HUse_Q").GetValue<bool>())
                        if (_Q.IsReady())
                            if (_Q.GetPrediction(QTarget).Hitchance >= HitChance.Medium)
                                _Q.CastIfHitchanceEquals(QTarget, _Q.GetPrediction(QTarget).Hitchance, true);
                }
