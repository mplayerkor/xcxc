    ...
    class Bard
    {
        ...
        private void SkillSet()
        {
            try
            {
                ...
                _R.SetSkillshot(0.5f, 120f, 1400f, false, SkillshotType.SkillshotLine);
            }
            catch (Exception)
            {
        ...
        private static void OnProcessSpell(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            try
            {
                ...
                if (_MainMenu.Item("Braum_AutoE").GetValue<bool>() && _E.IsReady())
                {
                    ...
                    var myteam = HeroManager.Allies.Where(f => f.Distance(Player.Position) < 120);
                    var count = myteam.Count(f => enemyskill.IsInside(f.Position));
                    if (args.Target != null)
                    {
                        if (args.Target.IsMe && Player.HealthPercent < 20)
                        {
                            _E.Cast(args.Start, true);
                        }
                        else if (args.Target.Position.Distance(Player.Position) < 120)
                        {
